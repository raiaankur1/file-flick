...
def login(request):
    ...
    response = Response({"token": token.key, "user": {
...
def guest_upload(request):
    try:
        ...
        if serializer.is_valid():
            ...
            object_key = 'uploads/{}.{}'.format(
                serializer.data['url'], filename.split('.')[-1])
            ...
            short_url = "https://file-flick.up.railway.app/FMS/download/" + \
                str(url_object.short_uid)
...
def download(request, shortuid):
    if request.method == "GET":
        ...
        if url_object is None:
            ...
            bucket_name = 'file-flick'
            object_key = 'uploads/{}.{}'.format(
                url_object.uid, filename.split('.')[-1])
